...
DESCRIPTION
       ...
       vmstat 报告有关进程，内存，页，块IO，traps，磁盘以及 cpu 活动的信息。

       The first report produced gives averages since the last reboot.  Additional reports give information on a sampling period of length delay.  The process and mem‐
       ory reports are instantaneous in either case.
       生成的第一份报告给出了自上一次重启以来的平均值。其他报告提供有关长度延迟的采样周期的信息。在任何一种情况下，进程和内存的报告都是即时的。
...
OPTIONS
       ...
       delay  更新之间的延迟，以秒为单位。如果没有指定 delay，仅有一个报告打印出自上一次启动后的平均值。

       count  Number of updates.  In absence of count, when delay is defined, default is infinite.
       count  更新次数。如果没有 count，则定义 delay 时，默认为无限。
       ...
       -f, --forks
              ...
              -f 选项显示自启动以来 forks 的次数。包括 fork，vfork 以及 clone 系统调用，并且与已创建的总任务数相等。每个进程由一个或多个任务表示，具体取决于线程使用情况。此显示不重复。
       ...
       -w, --wide
              Wide  output  mode (useful for systems with higher amount of memory, where the default output mode suffers from unwanted column breakage).  The output is
